...
> 💡 **PUT vs PATCH**<br>
> 간단히 비교하면, PUT은 전체 수정 / PATCH는 일부 수정이라고 볼 수 있다.
> - `PUT` : 입력하지 않은 값에 대해서는 NULL값으로 변하
> - `PATCH` : 입력되지 않은 값은 기존에 있던 값 유지

나는 변경될 필요가 없는 비밀번호나 생성일자 등이 수정 후 NULL값으로 변하면 안되기에 PATCH 메소드를 이용했다.

단, 찾아본 바에 따르면 수정 API를 작성할 경우에는 부분 수정을 위한 데이터만 요청을 보내기 위해 Body를 받는 DTO를 별도로 만들어두어야 한다고 한다. 이때 이 부분 데이터를 받는 DTO로는 새로운 Entity를 생성하는 것이 아닌, 오직 부분 수정을 위한 데이터 준비 과정이다.

이러한 부분을 제대로 완벽하게 구현하지는 못했기 때문에 완전히 100% 패치의 기능을 사용한 것 같지는 않다.

다음 번에는 완벽하게 부분 수정을 할 수 있도록 PATCH를 사용하는 방법을 알아두어야겠다.

단, 게시글 수정이라는 게 input 칸에 원래 있던 데이터값이 보여져야 편리하기에 이는 front단과도 관련이 있을 듯 하다. 추후 더 학습해보아야겠다.
